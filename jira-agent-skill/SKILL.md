...
Call the Jira tools directly from Python. This is faster and gives you full control:

```python
import sys
sys.path.insert(0, "C:/agents/JIRA Skill")
...
# Returns: key, summary, description, status, priority, labels, components, reporter

# 2. Get all child stories/tasks
children = get_epic_children("AA-1")
# Returns: list of children with key, summary, issue_type, status, priority, story_points

# 3. For each child, fetch full details + comments
for child in children["children"]:
    details = get_issue_details(child["key"])
    # Returns: full description, acceptance_criteria, subtasks, linked_issues
    comments = get_issue_comments(child["key"])
    # Returns: list of comments with author, body, created date
```
...
# Returns: list of projects with key, name, project_type, style
# Ask user to pick one if not specified

# 2. Check available issue types
types = get_project_issue_types("AA")
# Returns: project info + list of issue types (Epic, Story, Task, etc.)
# Confirm Epic, Story, Task are available before proceeding
...
epic_key = epic["issue_key"]  # e.g., "AA-15"
...
story = create_story(
    ...
    epic_key=epic_key,      # Links to the parent epic
    priority="High",
...
task = create_task(
    ...
    description="Install stripe-python, configure...",
    epic_key=epic_key,
    priority="High"
)
```
...
|------|--------|-------------|
| `list_projects()` | GET | Lists all accessible Jira projects |
| `get_project_issue_types(project_key)` | GET | Discovers issue types for a project |
| `get_epic_details(epic_key)` | GET | Fetches epic summary, description, status |
| `get_epic_children(epic_key)` | GET | Retrieves all children (stories/tasks) under an epic |
| `get_issue_details(issue_key)` | GET | Full details: description, AC, subtasks, links |
| `get_issue_comments(issue_key)` | GET | All comments on an issue |
| `create_epic(project_key, summary, ...)` | POST | Creates a new Epic |
| `create_story(project_key, summary, ..., epic_key)` | POST | Creates a Story linked to an epic |
| `create_task(project_key, summary, ..., epic_key)` | POST | Creates a Task linked to an epic |

All tools return `{"status": "success", ...}` or `{"status": "error", "error_message": "..."}`.

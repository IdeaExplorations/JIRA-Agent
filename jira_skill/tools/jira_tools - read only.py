...
def get_epic_children(epic_key: str) -> dict:
    ...
    # Try "Epic Link" first (classic projects), then fall back to "parent" (next-gen)
    jql_queries = [
    ...
    for jql in jql_queries:
        ...
        while True:
            ...
            result = _jira_request("/rest/api/2/search", params=params)
    ...
    return {
        "status": "error",
        "error_message": f"Could not find child issues for epic {epic_key}. Tried both 'Epic Link' and 'parent' JQL queries.",
    }
